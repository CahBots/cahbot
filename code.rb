...
bot.command(%i[eightball eball 8ball], help_available: false, min_args: 1) do |event|
...
bot.command(:rnumber, help_available: false, min_args: 2, max_args: 2) do |_event, min, max|
...
bot.command(:say, help_available: false, required_permissions: [:manage_messages], min_args: 1) do |event, *args|
  event.message.delete
  args.join(' ').to_s
end

bot.command(%i[reverse rev], help_available: false, min_args: 1) do |_event, *args|
  args.join(' ').to_s.reverse
end

bot.command(%i[uinfo userinfo user me], help_available: false, max_args: 0) do |event|
...
bot.command(%i[servercount servcount], help_available: false, max_args: 0) do |event|
...
bot.command(%i[cmds commands], chain_usable: false, max_args: 0) do |event|
...
bot.command(%i[subscribe sub]) do |event|
  if event.channel.pm? == true
...
bot.command(%i[unsubscribe unsub]) do |event|
  if event.channel.pm? == true
